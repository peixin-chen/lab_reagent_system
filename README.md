...
- **试剂搜索**：按试剂名称或 CAS 号模糊搜索，搜索结果同样支持入库/领用操作。
...
| `Reagent` | `reagents` | 试剂条目：名称、CAS 号、规格、数量、所属试剂柜、最近入库/领用时间 |
| `StockRecord` | `stock_records` | 每次入库或领用的操作记录，试剂删除后记录保留（外键置空） |
| `DepletionAlert` | `depletion_alerts` | 试剂耗尽提醒：保存耗尽试剂的名称/CAS/规格/所属试剂柜 |
...
| `/search` | GET | 搜索页。接收 URL 参数 `q`，用 `ilike` 对名称和 CAS 号做不区分大小写的模糊查询 |
...
| 路由 | 功能说明 |
|------|----------|
| `/reagent/add` | 新增试剂。若名称+规格+CAS 完全匹配已有试剂，则改为执行入库操作，不重复建条目 |
...
4. **试剂数据表格**：展示当前试剂柜的所有试剂，数量 ≤ 5 的行标黄色警告背景，每行有入库/领用/（管理员）删除按钮。
...
结构与主页试剂表格类似，额外增加了所属试剂柜列，并在每个操作表单中携带 `from_search` 隐藏字段，使入库/领用操作后能返回搜索结果而非主页。
