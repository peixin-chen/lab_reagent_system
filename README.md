...
> 基于 Flask + SQLite + Bootstrap 5 的实验室试剂库存管理系统，支持用户登录注册、试剂柜分类管理、试剂入库/领用、耗尽提醒、CSV 导出，以及管理员后台管理功能。
> v1.4 新增试剂备注（note）信息，并开放普通用户修改、删除试剂的权限。
...
> 说明：从 v1.4 开始，试剂的编辑与删除不再仅限管理员，普通登录用户也可操作。
...
- `/reagents/delete/<int:reagent_id>`
...
### 8.4 `delete_reagent()`：删除试剂

v1.4 中，普通登录用户也可以删除试剂。

删除逻辑：

1. 查询目标试剂
2. 将历史 `StockRecord` 中关联的 `reagent_id` 置空，保留历史记录
3. 删除当前试剂条目
4. 返回首页或搜索页

这个设计保证了“删除当前库存”与“保留历史操作记录”可以同时成立。
...
从 v1.4 开始，补货恢复时也会同步恢复 `note` 备注信息。
...
- `GET /`
- `GET /search`
- `GET /export/cabinet/<int:cabinet_id>`
- `GET /user-doc`

### 试剂相关

- `POST /reagent/add`
- `POST /reagent/edit/<int:reagent_id>`
- `POST /reagents/delete/<int:reagent_id>`
- `POST /reagent/stock_in/<int:reagent_id>`
- `POST /reagent/withdrawal/<int:reagent_id>`
- `POST /reagent/restock_alert/<int:alert_id>`

### 管理员相关

以 `admin.py` 中定义为准，主要包括：
...
- `models.py` 中的 `Reagent`
- 对应表单模板
- `reagent.py` 中新增 / 编辑 / 补货逻辑
- 若要保留历史快照，也应同步修改 `StockRecord` 与 `DepletionAlert`

### 2. 想修改搜索条件

修改位置：

- `blueprints/main.py` 中的 `search()`

### 3. 想增加权限控制

修改位置：

- `blueprints/reagent.py`
- `templates/index.html`
- `templates/search.html`

例如若希望恢复“只有管理员可编辑/删除”，可重新加入 `current_user.is_admin` 判断并同步调整模板按钮显示逻辑。

### 4. 想增加备注相关能力

当前 v1.4 已支持备注录入、展示、编辑与补货恢复。
若后续要进一步扩展，可考虑：
...
- 所有试剂新增备注 `note` 字段
- `StockRecord` 新增备注快照字段
- `DepletionAlert` 新增备注字段
- 新增试剂时支持填写备注
- 编辑试剂时支持修改备注
- 首页和搜索页支持显示备注
- 普通登录用户现在也可以编辑试剂
...
- 增加数据库迁移支持（如 Flask-Migrate）
- 将备注字段加入 CSV 导出
- 搜索支持备注全文检索
- 增加操作日志导出
- 增加试剂图片 / SDS 附件上传
- 增加更细粒度的权限控制（例如“仅管理员可删除”）
- 增加库存预警阈值，而不只是耗尽提醒
- 增加部署配置（Gunicorn / Nginx / Docker）