...
def _save_record(reagent_id, name, cas, spec, cabinet_name, qty, op_type):
    """保存入库/领用记录"""
    record = StockRecord(
...
def add():
    ...
    if existing:
        ...
        _save_record(existing.id, existing.name, existing.cas_number,
        ...
        db.session.flush()
        _save_record(reagent.id, name, cas, spec, cabinet.name, quantity, 'in')
        db.session.commit()
...
def stock_in(reagent_id):
    ...
    _save_record(reagent.id, reagent.name, reagent.cas_number,
...
def withdrawal(reagent_id):
    ...
    _save_record(reagent.id, r_name, r_cas, r_spec, cabinet_name, quantity, 'out')
